...
from logparser.ASParser.SimAndSplitFirst import sim_log_node, find_brackets,sim_log2log,sim_node_node,SplitFirstLayer
...
class Node:
    ...
    def canSplit(self):
        ...
        if "<->" in self.values:
            return False
        if len(self.values) >= round(100.0*self.score):
            return False
...
class Tempalte_tree:
    ...
    def merge_log_node(self, logid, Split_words_bySpace,delimiters,brackets, sim_score):
        ...
        self.sim_tau=self.sim_tau+(sim_score*0.8-self.sim_tau)*0.1

        length = len(self.nodeList)
        # self.logid_sim[logid]= sim_score
        for i in range(length):
...
class LogParser:
    def __init__(self, log_format, indir='./', outdir='./result/', st=0.75, size=None):
        self.path = indir
        self.st = st
    ...
    def mergeTwoClusters(self, cluster1, cluster2,score, align):
        ...
        align2={}
        for tuple in align:
            align1.append(tuple[0])
            align2[tuple[0]]=tuple[1]
        for i in range(len(cluster1.nodeList)):
            if(i in align1):
                if(i==len(cluster1.nodeList)-1):
                    if(pos2==len(cluster2.nodeList)-1):
                        mergeTreeNodes(cluster1.nodeList[i], cluster2.nodeList[pos2])
                    else:
                        length = len(cluster2.nodeList) - pos2-1
                        for j in range(length):
                            horizontalMergeTwoNodes(cluster2.nodeList[pos2],cluster2.nodeList[pos2+j+1])
                        mergeTreeNodes(cluster1.nodeList[i], cluster2.nodeList[pos2])
                    break
                if(pos2==align2[i]):
                    mergeTreeNodes(cluster1.nodeList[i], cluster2.nodeList[pos2])
                    ...
                    length=align2[i]-pos2
                    for j in range(length):
                        horizontalMergeTwoNodes(cluster2.nodeList[pos2],cluster2.nodeList[pos2+j+1])
                    mergeTreeNodes(cluster1.nodeList[i], cluster2.nodeList[pos2])
                    pos2=align2[i]+1
            else:
                cluster1.nodeList[i].word='<->'
                cluster1.nodeList[i].makeLeaf()
                cluster1.nodeList[i].values= set()
        return cluster1
    ...
    def find_candidateClu_Align(self, words_set):
        count={}
    ...
    def parse(self, logName):
        ...
        self.outputResult(self.logClusters)
    ...
    def addClusterToInvertTable(self, newCluster, cid):
        ...
        newCluster.min_score_demand-=scoreFirstLayer
    ...
    def buildLCSGraph(self):
        ...
        for cid, cluster in enumerate(self.logClusters):
            ...
            for other_id in candidates:
                ...
                if score < 0.5 or score < self.st-0.2 or len(cluster.nodeList)<5 or len(other_cluster.nodeList)<5:
                    continue
    ...
    def graph_iterate(self):
        ...
        graph.sort(key=lambda edge:edge.w, reverse=True)

        for edge in graph:
            cluster1 = self.logClusters[edge.u]
            cluster2 = self.logClusters[edge.v]
            if cluster2.removed or cluster1.removed:
                continue
            if edge.w < (cluster1.sim_tau+cluster1.min_sim)/2:
                continue
            self.mergeTwoCluWithEqLength(cluster1, cluster2, edge.w)
        ...
        for cluster in self.logClusters:
            if cluster.removed:
                continue
            self.addClusterToInvertTable(cluster, cid)
            cid += 1
            newClusters.append(cluster)
        return newClusters
    ...
    def lcsGraph_iterate(self):
        lcsGraph = self.buildLCSGraph()
        self.lcsGraphSize = len(lcsGraph)
        lcsGraph.sort(key=lambda edge:edge.w, reverse=True)
        father={}
        for edge in lcsGraph:
            cluster1 = self.logClusters[edge.u]
            cluster2 = self.logClusters[edge.v]
            if(len(cluster1.nodeList)<3 or len(cluster2.nodeList)<3):
                continue
            cid1=edge.u
            while(cid1 in father.keys()):
                cid1=father[cid1]
            cid2 = edge.v
            while (cid2 in father.keys()):
                cid2 = father[cid2]
            if(cid1==cid2):
                continue
            if edge.w < (cluster1.sim_tau+cluster1.min_sim)/2:
                continue
            cluster1=self.logClusters[cid1]
            cluster2 = self.logClusters[cid2]
            aligned_nodes,sim=alignTwo(cluster1.nodeList,cluster2.nodeList)
            self.mergeTwoClusters(cluster1, cluster2, sim, aligned_nodes)
            father[cid2]=cid1
        ...
        for i in range(len(self.logClusters)):
            if(i in father.keys()):
                continue
            self.addClusterToInvertTable(self.logClusters[i], cid)
            cid += 1
            newClusters.append(self.logClusters[i])
        return newClusters


    def mergeAndSplit(self):
        # print("initial clusters: " + str(len(self.logClusters)))
        # print("iterate with equal Length")
        self.logClusters = self.graph_iterate()
        # print("after equal Length Merge : " + str(len(self.logClusters)))
        # print("\niterate with LCS")
        self.logClusters = self.lcsGraph_iterate()
        # print("after LCS Merge: " + str(len(self.logClusters)))
        newClusterList =[]
        for cluster in self.logClusters:
    ...
    def outputResult(self, logClustL):
        ...
        for i in range(11):
            map[i]=0
        print('Templates count :', len(logClustL))
        ...
        self.df_log.to_csv(os.path.join(self.savePath, filename + '_structured.csv'), index=False,
                           encoding=en)
        df_event.to_csv(os.path.join(self.savePath, filename + '_templates.csv'), index=False, encoding=en)
