...
from logparser.ASParser.sim import sim_log_node, find_brackets,sim_log2log,sim_node_node,SplitFirstLayer
...
class LogParser:
    ...
    def find_candidateClu_Align(self, words_set,newClusters,selfnode):
        count={}
        ...
        temp = sorted(count.items(), key=lambda x: x[1], reverse=True)
        for id,score in temp:
            if(score >= newClusters[id].min_score_demand or score >= selfnode.min_score_demand):
                candidate_ids.append(id)
                scores.append(score)
        return candidate_ids, scores
    ...
    def threshold_adjustment(self):
        for cluster in self.logClusters:
            cluster.min_score_demand=cluster.min_score_demand+cluster.min_sim-self.tolerance-cluster.sim_tau
    ...
    def search_LCS(self,cid,cluster,newClusters):
        ...
        max_score = 0
        alignress=None
        candidates, sameTkCount = self.find_candidateClu_Align(cluster.trivial_rep,newClusters,cluster)
